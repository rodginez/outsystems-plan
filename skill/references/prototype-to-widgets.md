...
## 31. A Mentor "0 errors, applied" claim proves nothing about the published app — verify against the model with `publish` in between, wait for indexing, and read the actual publish error, not just Mentor's summary
...
## 32. `max-width` on a flex item does nothing without a `width` (or `flex-grow`) pushing the element toward it

A container styled with `max-width: 960px` inside a `display: flex` parent (with the container itself also `display: flex`, no `width` set) will not visibly grow to that width — it stays sized to its content, because `max-width` is only an upper *bound*, not a target. Without something driving the element toward that bound — `width: 100%` (clamped by the parent's available space) or `flex-grow: 1` — the box just shrinks to fit its content and the `max-width` rule sits there doing nothing, with 0 validation errors and no visual change to signal the mistake. This surfaced as a live "why is the screen still narrow after raising max-width" report — `getComputedStyle` on the element showed `max-width: 960px` had applied correctly, but `width`/`getBoundingClientRect()` still reported the old, narrower value, which is the tell: if a widened `max-width` doesn't move the rendered width at all, check whether the element has an explicit `width` (or `flex-grow`) — a flex item defaults to `flex: 0 1 auto`, sized to content, and `max-width` alone can't override that default. Fix: add `width: 100%;` alongside the `max-width` on the same rule.
