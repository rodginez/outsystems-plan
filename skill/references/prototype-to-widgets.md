...
description: >
  ...
  Strictly widget/CSS/DOM lessons — for Server Action, entity, aggregate, and
  Mentor-process gotchas (including ones that surface during live debugging,
  not just wave authoring) see `backend-and-data-gotchas.md` instead.
---
...
## 23. A `RadioButtonGroup`'s `OnChange` cannot reliably read the just-clicked value for any row but the first, inside a list
...
## 24. A dynamically-computed CSS `width`/`height` that never renders — check `display` before the value
...
## 25. Ad-hoc debug buttons for one-off maintenance actions (reseed, backfill) should become a permanent "Dev tools" area, not an add-then-remove cycle
...
## 26. A filter dropdown sourced from a full historical table needs an explicit "actually used" scope, or it silently grows unusable
...
## 27. `max-width` on a flex item does nothing without a `width` (or `flex-grow`) pushing the element toward it
